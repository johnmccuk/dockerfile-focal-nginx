...
RUN DEBIAN_FRONTEND=noninteractive apt update -y && apt-get install software-properties-common -y && add-apt-repository ppa:ondrej/php -y && apt update -y && DEBIAN_FRONTEND=noninteractive apt install -y vim nginx nodejs npm \
    supervisor php7.4-fpm php7.4-mbstring php7.4-zip php-xdebug php-memcached php-imagick libssh2-1 libssl1.0.0 php-curl php-ssh2 php7.4-mysql php7.4-xml composer rsyslog zip unzip && apt clean && service php7.4-fpm start && usermod -u 1000 www-data
